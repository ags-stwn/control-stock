...
// ==============================================
// CEK BY CODE PAGE - FIXED (Case Insensitive)
// ==============================================
...
class _CekByCodePageState extends State<CekByCodePage> {
  ...
  try {
    ...
    // ========== AMBIL SEMUA DATA GWS DENGAN PAGINATION ==========
    List<Map<String, dynamic>> allGws = [];
    int start = 0;
    const int batchSize = 1000;
    bool hasMore = true;
    
    print("🔄 Mengambil semua data GWS...");
    ...
    while (hasMore) {
      ...
      print("  Mengambil baris $start - ${start + batchSize - 1}: ${response.length} baris");
    ...
    // ========== AMBIL SEMUA DATA NON GWS DENGAN PAGINATION ==========
    List<Map<String, dynamic>> allNonGws = [];
    start = 0;
    hasMore = true;
    
    print("🔄 Mengambil semua data NON GWS...");
    ...
    print("✅ TOTAL GWS: ${allGws.length} baris");
    print("✅ TOTAL NON GWS: ${allNonGws.length} baris");
    
    // Cek apakah ada data C45830 di GWS
    int found = 0;
    for (var item in allGws) {
      if (item['code'] == 'C45830') {
        found++;
      }
    }
    print("🎯 Data dengan code C45830 di GWS: $found baris");
  ...
  List<Map<String, dynamic>> get filteredData {
    ...
    String queryLower = searchCode.toLowerCase().trim();
    
    print("🔍 Searching for: '$query' in GWS (${allDataGws.length} rows)");
    ...
    print("  NON GWS matches: $nonGwsMatch");
    print("📊 Total matches: ${results.length}");
    
    // Urutkan berdasarkan source
            ...
            child: Column(
              children: [
                TextField(
                  ...
                  autofocus: true,
                  decoration: InputDecoration(
                    hintText: "Cari berdasarkan Code... (contoh: 4583, C45830, 12510)",
                    prefixIcon: const Icon(Icons.search),
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                    filled: true,
                    fillColor: Colors.white,
  ...
  Widget _buildBody() {
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text("Contoh: 4583, C45830, 12510", style: TextStyle(fontSize: 12, color: Colors.grey)),
            SizedBox(height: 8),
            Text("Total GWS: 7160 baris | Total NON GWS: 20466 baris", 
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed: () {
                // Coba cari dengan format berbeda
                if (searchCode.length >= 4 && !searchCode.startsWith('C')) {
                  _codeController.text = 'C$searchCode';
                  setState(() => searchCode = 'C$searchCode');
                }
              },
              icon: const Icon(Icons.lightbulb, size: 16),
              label: Text("Coba dengan format C$searchCode"),
              style: ElevatedButton.styleFrom(
    ...
    int totalQty = filteredData.fold(0, (sum, item) => sum + ((item['qty'] ?? 0) as int));
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              Text("${filteredData.length} baris | Total: $totalQty Pcs",
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              _buildHeaderCell("Sumber", flex: 2),
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final bool isGws = item['source'] == 'GWS (NK01)';
                ...
                child: Row(
                  children: [
                    ...
                    _buildCell(
                      ...
                      color: isGws ? Colors.blue.shade700 : Colors.orange.shade700,
                    ),
                    _buildCell(
                    ...
                    _buildCell(item['lokasi'] ?? '-', flex: 3, fontSize: 11),
                    _buildCell(item['weekly'] ?? '-', flex: 2),
                    _buildCell(
                      "${item['qty'] ?? 0}", 
                      flex: 1, 
  ...
  try {
    ...
    while (hasMore) {
      final response = await Supabase.instance.client
          .from('inventory_gws')
          .select()
          .order('code')  // Urutkan berdasarkan code A-Z
          .range(page * pageSize, (page + 1) * pageSize - 1);
      
      allGws.addAll(List<Map<String, dynamic>>.from(response));
      print("  Halaman $page: ${response.length} baris");
    ...
    print("🎯 Data C45830 di GWS: $foundGws baris");
    
    // Debug: sample 5 code pertama
    print("📝 Sample 5 code GWS:");
    for (int i = 0; i < (allGws.length > 5 ? 5 : allGws.length); i++) {
      print("   ${i+1}. ${allGws[i]['code']}");
    }
            ...
            child: ExpansionTile(
              ...
              children: items.map((item) {
                return ListTile(
                  ...
                  title: Text((item['code'] ?? '-').toString().toUpperCase(), style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text("Weekly: ${item['weekly'] ?? '-'} | Qty: ${item['qty']} Pcs"),
                  trailing: Chip(
                    label: Text(item['source'] ?? ''),
                    backgroundColor: item['source'] == 'GWS' ? Colors.blue.shade100 : Colors.orange.shade100,
                  ),
                );
              }).toList(),
            ),
